...
export class SimpleNetworkLatencyTest implements Scenario {
  ...
  async run(controller: ScenarioControllerInterface): Promise<void> {
    ...
    // Patch: kubectl patch daemonset <daemonset-name> -n <namespace> -p '{"spec": {"template": {"spec": {"nodeSelector": {"non-existing": "true"}}}}}'
    console.log(
      'Remove the pod to test K8 contollers respons ewhen the pod is not available'
    );

    //remove laterncy
    await (
      provider.instanceController as SplitEDCInstance
