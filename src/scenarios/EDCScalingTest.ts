...
export class EDCScalingTest implements Scenario {
  scenario_name =
    'EDC Scaling Test - Concurrent Consumer Connectors on Single Asset';
  ...
  static async runBatch(
    ...
    const results = await Promise.allSettled(
      consumers.map(async (consumer, i) => {
        ...
        await EDCScalingTest.negotiateAndTransfer(
          controller,
          consumer,
          batchSize,
          i
        ...
        controller.log(
          'info',
          `  ✓ Consumer ${i} completed in ${consumerTimer.getTime()}ms after a timeout of ${
